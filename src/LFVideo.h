...
public:
    ...
    cv::Mat SingleView(int _frameId, float _uf, float _vf);
...
private:
    ...
    void Clear();
    cv::Mat SingleView(int _frameId, int _viewId);
    cv::Mat CentralView(int _frameId);

    void CheckFrameId(int _frameId);
    void CheckViewId(int _viewId);
    void CheckAperture(float _aperture);
    cv::Mat RenderLFFull(int _frameId, float _alpha=0.5);
    cv::Mat RenderLFWeighted(int _frameId, float _alpha=0.5, float _aperture=1.0);
private:
    int m_numFrames;
    int m_numViews;
    int m_frameHeight;
